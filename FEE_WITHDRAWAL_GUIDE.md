# fees Withdrawal Guide (MVP - Manual Verification)

## Overview

The contract allows the owner to withdraw house feess (fees collected from games), but **you must manually verify** that you're not draining player funds.

---

## ⚠️ CRITICAL: Manual Verification Required

**Before every `withdrawFees()` call:**
...
4. **Withdraw only up to `safeWithdrawal`:**
   ```javascript
   await vault.withdrawFees(safeWithdrawal);
   ```
...
// Withdraw
await vault.withdrawFees(ethers.parseUnits('15', 6));
```

**Result:**
- Contract: 135 mUSD
- Database: 135 chips
- Owner wallet: +15 mUSD fees
...
const tx = await vaultWithSigner.withdrawFees(safeWithdrawal);
...
4. **Am I leaving a buffer?** (Optional: withdraw 90% of calculated fees for safety)

### Red Flags (DON'T WITHDRAW IF):

- ❌ You haven't checked the database in the last hour
- ❌ You're unsure about the chip balance query
- ❌ The calculated fees seems too high (double-check math)
...
async function withdrawFees() {
  ...
  if (safeWithdrawal <= 0n) {
    console.log('No fees available (contract balance <= chip liability)');
    return;
  }
  
  console.log('Available fees:', ethers.formatUnits(safeWithdrawal, 6), 'mUSD');
  
  // 4. Optional: Apply safety margin (withdraw 95% of fees)
  ...
  console.log('\n⚠️  About to withdraw fees. Press Ctrl+C to cancel, or wait 5 seconds...');
  await new Promise(resolve => setTimeout(resolve, 5000));
  
  const tx = await vault.withdrawFees(withdrawAmount);
...
```bash
cd packages/backend
npx tsx scripts/withdraw-fees.ts
```
...
- Contract enforces: `withdrawFees()` only works if data is fresh (<1 hour)
...
**Q: Can players see how much fees I'm taking?**  
A: Yes, `FeesWithdrawn` events are on-chain and public. This is transparent by design.

**Q: Should I withdraw fees daily? Weekly?**  
A: Up to you. More frequent = more gas fees. Weekly is reasonable.

**Q: What if there's a big pending withdrawal?**  
A: Check `pendingWithdrawals` table before calculating fees. Subtract those amounts from available fees.
